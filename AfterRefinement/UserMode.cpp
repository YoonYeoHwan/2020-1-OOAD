...
void UserMode::buyWithPrescription() {
    cout << "------------------------------------------------------------" << endl;
    cout << "\n처방전을 제출해주세요.\n" << endl;
    cout << "------------------------------------------------------------" << endl;
    sleep(1);
    ...
    PC.paymentModeSelect(price);

    SalesAdaptor SA;
    SA.SalesChange(price);

    ReceiptCheck RC;
    if(RC.wantReceipt()) {
        UI ui;
        ui.printReceipt(price);
    }

    Log log;
    log.LogOthers(price, "처방전");
}
...
void UserMode::buyMedicine() {
    ...
    StockAdaptor STA;
    STA.StockChange(0, 0, v);
    
    Log log;
    log.LogMedicine(price, productsName, v);
}
...
void UserMode::buyMask() {
    ...
    if(number == 0) {
        ...
        if(t->tm_wday == 0 || t->tm_wday == 6) {
            ...
            cin >> personalID;
            cout << "------------------------------------------------------------\n" << endl;
            MaskAvailablityCheck MAC;
            if(MAC.checkAvailablity(personalID)) {
